...
/**
 * A request saved inside a collection. It is a shareable-request payload
 * (method, endpoint, headers, body, ...) with a display name, so it can be
 * loaded straight into a request tab via the existing shared-payload restore
 * path. Generated by tooling to pre-fill flows (e.g. a registration sequence).
 */
export type SavedRequest = ShareableLinkPayload & {
    id: string;
    name: string;
};
