...
const statusColors = [
    ...
    { name: 'Info', variable: '--color-info', foreground: '--color-info-foreground' },
];

const sidebarColors = [
    { name: 'Sidebar BG', variable: '--sidebar-background' },
...
<template>
    <div class="bg-background text-foreground min-h-screen space-y-12 p-8">
        <section>
            <h2 class="mb-6 border-b pb-2 text-2xl font-bold">Semantic Palette</h2>
            <div class="grid grid-cols-2 gap-6 md:grid-cols-3 lg:grid-cols-4">
                <div v-for="color in semanticColors" :key="color.name" class="space-y-2">
                    <div
                        class="flex h-24 flex-col items-center justify-center rounded-lg border p-4 text-center shadow-sm"
                        :style="{
                    ...
                    >
                        ...
                        <code class="mt-1 text-[10px] opacity-70">
                            {{ color.variable }}
                        </code>
                    </div>
                </div>
        ...
        <section>
            <h2 class="mb-6 border-b pb-2 text-2xl font-bold">Status Colors</h2>
            <div class="grid grid-cols-2 gap-6 md:grid-cols-3 lg:grid-cols-4">
                <div v-for="color in statusColors" :key="color.name" class="space-y-2">
                    <div
                        class="flex h-24 flex-col items-center justify-center rounded-lg border p-4 text-center shadow-sm"
                        :style="{
                            backgroundColor: `var(${color.variable})`,
        ...
        <section>
            <h2 class="mb-6 border-b pb-2 text-2xl font-bold">Sidebar System</h2>
            <div class="grid grid-cols-2 gap-6 md:grid-cols-3 lg:grid-cols-4">
                <div v-for="color in sidebarColors" :key="color.name" class="space-y-2">
                    <div
                        class="flex h-24 flex-col items-center justify-center rounded-lg border p-4 text-center shadow-sm"
                        :style="{
                            backgroundColor: `var(${color.variable})`,
