...
class ExtractApplicationRoutesActionFunctionalTest extends TestCase
{
    public function test_it_processes_routes_properly(): void
    {
        ...
        $routeFactoryMock = $this->mock(ExtractableRouteFactory::class, function (MockInterface $mock) {
            $mock
                ...
                ->withAnyArgs()
                ->andReturnUsing(fn (Route $route) => $route)
                ->times(3);
        });

        $schemaExtractorMock = $this->mock(RequestSchemaExtractor::class, function (MockInterface $mock) {
            $mock
                ->shouldReceive('extract')
                ->withAnyArgs()
                ->andReturnUsing(
                    fn (?Route $route) => new Schema([new StringSchemaProperty('foobar')]),
                )
                ->times(3);
        });
        ...
        $result->each(function (ExtractedRoute $extractedRoute) use ($routeFactoryMock, $schemaExtractorMock, $routes) {
            ...
            $schemaExtractorMock
                ->shouldHaveReceived(
                    'extract',
                    fn (?Route $routeArg) => $routeArg?->getName() === $originalRoute->getName(),
                );
        });
    }
    ...
    public function test_it_handles_extraction_errors_gracefully(): void
    {
        ...
        $this->mock(RequestSchemaExtractor::class, function (MockInterface $mock) use ($dummyFailingException) {
            $mock
                ->shouldReceive('extract')
                ->withAnyArgs()
        ...
        try {
            $result = $routeExtractorService->execute($routes);
        } catch (RouteExtractionInternalException $e) {
            $exception = $e;
        }
