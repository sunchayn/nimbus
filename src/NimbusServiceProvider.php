...
class NimbusServiceProvider extends PackageServiceProvider
{
    ...
    public function register(): void
    {
        ...
        $this->app->bind(CollectionStoreContract::class, function (Container $container): CollectionStoreContract {
            ...
            return match ($driver) {
                'json' => new JsonFileCollectionStore(
                    $container->make(Filesystem::class),
                    config('nimbus.collections.path') ?? base_path('nimbus/collections'),
                ),
                default => throw new \InvalidArgumentException(
                    sprintf('Unsupported Nimbus collections driver [%s]. Bind your own ', $driver).CollectionStoreContract::class.' implementation to add one.'
                ),
            };
        });
    }
